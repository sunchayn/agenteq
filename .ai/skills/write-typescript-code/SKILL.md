...
- **Definition**: a plugin-style config object for one instance of a pluggable concept. One file per instance, exporting a single `new Entity({...})`. Adding or refactoring an Agent definition also means updating the reference table in both [refactor-to-agenteq](../refactor-to-agenteq/SKILL.md)'s SKILL.md and [add-agenteq-to-boost-project](../add-agenteq-to-boost-project/SKILL.md)'s SKILL.md, an identical table duplicated in both, hardcoding each agent's paths and MCP shape since those skills run outside this repo with no access to the source.
