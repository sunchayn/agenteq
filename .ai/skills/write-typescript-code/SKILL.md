...
- **Definition**: a plugin-style config object for one instance of a pluggable concept. One file per instance, exporting a single `new Entity({...})`. Adding or refactoring an Agent definition also means updating the reference table in [refactor-to-agenteq](../refactor-to-agenteq/SKILL.md)'s SKILL.md, that table hardcodes each agent's paths and MCP shape since the skill runs outside this repo with no access to the source.
